...
> **범위 결정**: PRD-v2에서 Phase 1에 F11(태스크 보드)과 F1(대시보드 전체)을 포함하였으나, 실제 Phase 1에서는 F1~F4(세션 모니터링 핵심)만 우선 완료하였다. F11과 대시보드 전체 구현은 Phase 2에서 재평가한다. 현재 DashboardPage는 플레이스홀더 스텁 상태이다.
...
- [x] `~/.claude/projects/` 디렉토리 스캔 서비스 (메인 프로세스)
- [x] `history.jsonl` 파싱하여 세션 메타데이터 추출
- [x] IPC 핸들러: 세션 목록 조회
- [x] 세션 목록 UI 컴포넌트 (프로젝트별 그룹핑)
- [x] 세션 상태 표시 (활성/완료)
