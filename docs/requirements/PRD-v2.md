...
> - **Phase 1 범위 확장**: PRD v1의 F1~F4 → v2에서 F11(태스크 보드) 추가 *(계획. 실제 Phase 1에서는 F1~F4만 우선 완료, F11은 Phase 2에서 재평가)*
...
| **F17** | **스킬 모니터** | 프로젝트/글로벌/플러그인 스킬 목록. YAML frontmatter 파싱하여 name, description, tools, model 표시 | .claude/skills/*/SKILL.md, ~/.claude/skills/, plugins/ |
| **F18** | **에이전트 모니터** | 프로젝트/글로벌 에이전트 목록. YAML 파싱하여 name, description, tools, model 표시. 서브에이전트 사용 이력 연동 | .claude/agents/*.md, ~/.claude/agents/ |
| **F19** | **메모리 뷰어** | 프로젝트별 MEMORY.md 내용 표시 + 검색. 라인 수 경고(200줄 초과 시) | ~/.claude/projects/{path}/memory/MEMORY.md |
| **F20** | **훅/규칙/MCP 모니터** | Hooks: 이벤트별 그룹 + 실행 상태. Rules: 경로별 규칙 목록 + 내용 미리보기. MCP: 서버 목록 + 연결 상태. Permissions: allow/deny 목록 | .claude/settings.json, .claude/rules/*.md, .mcp.json, ~/.claude.json |
...
> 본 섹션에서는 v2에서 신규 추가된 F11~F20만 기술한다.
...
**목적**: Claude Code 커스텀 스킬(슬래시 커맨드)의 전체 목록과 상세 정보를 조회

**데이터 소스**:
- 프로젝트: `{project}/.claude/skills/{name}/SKILL.md`
- 글로벌: `~/.claude/skills/{name}/SKILL.md`
- 플러그인: `~/.claude/plugins/marketplaces/.../skills/`

**표시 정보** (YAML frontmatter 파싱):
- name, description, allowed-tools, model, disable-model-invocation
- 스코프 구분: 프로젝트 / 글로벌 / 플러그인
- 파일 경로, 크기, 수정일
...
**목적**: Claude Code 커스텀 서브에이전트의 전체 목록과 사용 이력

**데이터 소스**:
- 프로젝트: `{project}/.claude/agents/{name}.md`
- 글로벌: `~/.claude/agents/{name}.md`
- 플러그인: `~/.claude/plugins/marketplaces/.../agents/`

**표시 정보** (YAML frontmatter 파싱):
- name, description, tools, model, permissionMode
- 스코프 구분: 프로젝트 / 글로벌 / 플러그인
- JSONL에서 Agent tool_use의 subagent_type과 매칭하여 **사용 빈도** 표시

---

### F19. 메모리 뷰어

**목적**: 프로젝트별 MEMORY.md 내용 조회 및 건강 상태 표시
...
- 마크다운 렌더링 (Readout 참고)
- 라인 수, 파일 크기
- **경고**: 200줄 초과 시 "Claude 시스템 프롬프트에서 잘림" 경고 표시
- 프로젝트별 메모리 검색
...
**목적**: Claude Code의 훅, 규칙, MCP 서버, 퍼미션 설정을 통합 조회

**4개 탭으로 구성**:

**탭 1: Hooks**
- 데이터: `.claude/settings.json`의 hooks 섹션
- 이벤트별 그룹핑 (PreToolUse, PostToolUse, Stop, Notification 등)
- 각 훅: matcher, type(command/prompt/agent/http), command/prompt 내용
- 실행 상태: 성공(exit 0)/차단(exit 2)/실패(기타)

**탭 2: Rules**
- 데이터: `.claude/rules/*.md`
- 규칙 목록: 파일명 + YAML frontmatter의 paths 패턴 + 내용 미리보기
- 적용 범위 표시 (어떤 파일에 적용되는지)

**탭 3: MCP Servers**
- 데이터: `{project}/.mcp.json` + `~/.claude.json`
- 서버 목록: name, command, args, 스코프(프로젝트/글로벌)
- 연결 상태 표시 (Connected/Disconnected/Needs Auth)

**탭 4: Permissions**
- 데이터: `.claude/settings.json`의 permissions 섹션
- allow 목록 + deny 목록
- 글로벌 설정(`~/.claude/settings.json`) vs 프로젝트 설정 병합 표시
...
> **실제 Phase 1 결과**: 항목 1~6 완료 (F1 세션 목록, F2 실시간 감시, F3 타임라인, F4 도구 추적). 항목 7(F11 태스크 보드)과 항목 8(F1 대시보드 전체)은 미구현 → Phase 2로 이관.
